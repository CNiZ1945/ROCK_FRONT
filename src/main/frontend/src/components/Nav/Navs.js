...
function Navs() {
    ...
    const updateScroll = () => {
        setScrollPosition(window.scrollY || document.documentElement.scrollTop);
        console.log('스크롤 위치 업데이트:', scrollPosition);
    };
        ...
        <NavWrapper scrollposition={scrollPosition}>
            <MenuWrapper>
                <LogoImg
                    ...
                    onClick={() => goToPage(`/`)}
                />
                <MenuName onClick={() => goToPage(`/`)} scrollposition={scrollPosition}>홈</MenuName>
                <MenuName onClick={() => goToPage('/user/Recommend')} scrollposition={scrollPosition}>NEW!추천콘텐츠</MenuName>
                <MenuName onClick={() => goToPage('/user/notice')} scrollposition={scrollPosition}>공지사항</MenuName>
                <MenuName onClick={() => goToPage('/user/mypage')} scrollposition={scrollPosition}>마이페이지</MenuName>
                <MenuName onClick={() => goToPage('/admin/movieList')} scrollposition={scrollPosition}>관리자</MenuName>
            </MenuWrapper>
            ...
            <IconWrapper>
                <IconImg onClick={() => handleLogout()} alt="loginIcon" src={loginIcon} scrollposition={scrollPosition} />
                <IconImg onClick={() => goToPage('/user/mypage')} alt="Profile" src={Profile_1} scrollposition={scrollPosition} />
            </IconWrapper>
        </NavWrapper>
    );
}
...
const NavWrapper = styled.div`
    ...
    background: ${props => (props.scrollposition > 100 ? '#fff' : 'rgba(225, 225, 255, 0.1)')};
    transition: 0.2s ease-out;
    z-index: 100; /* Ensure Navs is on top */
`;
...
const LogoImg = styled.img`
    width: 130px;
    cursor: pointer;
    filter: ${props => (props.scrollposition > 100 ? 'invert(100%)' : 'invert(0%)')};
`;
...
const SearchInput = styled.input`
    ...
    &:focus {
        ...
        font-weight: 500;
        color: ${props => (props.scrollposition > 100 ? '#000' : 'white')};
        background: ${props => (props.scrollPosition > 100 ? '#fff' : 'rgba(249, 249, 249, 0.1)')};
        transition: 0.3s ease-out;
    }
`;
...
const IconImg = styled.img`
    width: 25px;
    cursor: pointer;
    filter: ${props => (props.scrollposition > 100 ? 'invert(0%)' : 'invert(100%)')};
`;

...
const PointsContent = ({ memNum }) => {
    ...
    const fetchPointsContentList = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const response = await api.get(`/user/personal/recommend/${memNum}/points-content`, {
        ...
        <SectionContainer>
            {recommendations.length > 0 ? (
                recommendations.map((item, index) => (
                    <SectionA key={index}>
                        <Link to={`/user/MoviePage/${item.movieId}`}>
                            <img
                                src={item.mainPosterUrl?.posterUrls || '/default-poster.jpg'}
                                alt={item.movieTitle}
                                onError={(e) => {
